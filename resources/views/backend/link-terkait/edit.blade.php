    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header border-bottom-0">
                <h3 class="card-title">Update {{ucfirst(request()->segment(2))}}</h3>
            </div>
            <div class="card-body">
                <form action="{{route('link-terkait.update', $data->id)}}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="row mb-4">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-10">
                            <input type="url" class="form-control @error('link') is-invalid @enderror" name="link" placeholder="URL" value="{{$data->link}}">
                            @error('link')
                            <span class="text-red">{{$message}}</span>
                            @enderror
                        </div>
