        ...
        <div class="card ">
            <div class="card-header">
                ...
                <div class="card-options">
                    <a href="{{route('mata-kuliah.create')}}" class="btn btn-primary">{{__('Add New')}}</a>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    {{-- Searching --}}
                    <form method="get">
                        <input name="keyword" class="form-control" placeholder="Search for anything..." type="search" value="{{ request()->get('keyword', '') }}">
                    </form>
                    <table id="data-table" class="table table-bordered text-nowrap mb-0 my-4">
                        <thead class="border-top">
                            <tr class="text-center">
                                <th class="bg-transparent border-bottom-0">Kode MK</th>
                                <th class="bg-transparent border-bottom-0">Nama MK</th>
                        ...
                        <tbody>
                            @foreach ($data as $d)
                            <tr>
                                <td class="align-middle"><strong>{{$d->kode}}</strong></td>
                                <td class="align-middle">{{$d->name}}</td>
                                <td class="align-middle text-center">
                                    <div class="row">
                                        <div class="col-6">
                                            <a class="btn btn-primary btn-sm rounded-11 mx-2 btn-block"
                                                data-bs-toggle="tooltip" href="{{route('mata-kuliah.edit', $d->id)}}"
                                        ...
                                        <div class="col-6">
                                            <form id="delete-item" action="{{route('mata-kuliah.destroy', $d->id)}}"
                                                method="post">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit"
